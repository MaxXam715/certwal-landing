class Modal {
    ...
    template() {
        ...
        let html = `
        <div class="cw-modal `+classModal+`" direction="`+this.data.mode+`" id="${idModal}" data-id="${idModal}">
    ...
    events() {
        ...
        this.modal.mousedown((e) => {
            if($(e.target).closest(".cw-modal").length && $(e.target).closest(".container-body").length == 0 && this.data.closeBackground != false && !$(e.target).hasClass('.container-body')) {
                this.closeModal();
            }
        });
        ...
        $(document).on('keydown', (e) => {
            if (e.keyCode  === 27 && $('.cw-modal').length > 0 && this.data.esc != false) {
                this.closeModal();
            }
        });
    }
    // закрыть функция
    closeModal() {
        if($(".cw-modal").last().data('id') == this.modal.data('id')){
            ...
            this.modal.removeClass('open').find('.container-body').on('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend webkitAnimationEnd oanimationend msAnimationEnd animationend', function(e) {
                this.closest('.cw-modal').remove();
                if ($('.cw-modal.open').length == 0) {
